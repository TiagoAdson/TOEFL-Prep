...
export default function TutorHumano() {
  ...
  async function analyzeWithAI() {
    if (!text.trim() || !HAS_AI) return
    setAnalyzing(true)
    setResult(null)
    setSaved(false)
    try {
      const prompt = `Você é um assistente pedagógico especializado em inglês acadêmico (TOEFL).
Um aluno colou o material do seu professor particular abaixo. Analise e:
...
Responda SOMENTE em JSON válido:
{
  ...
  async function saveMaterial() {
    ...
    await supabase.from('tutor_materials').insert({
      user_id: user.id,
      title: title.trim() || 'Material do Tutor',
      content_text: text,
  ...
  return (
    <div>
      ...
      <p style={S.pageDesc}>
        Cole aqui o material do seu professor particular. A IA irá identificar os módulos cobertos
        e gerar exercícios de <strong>homework</strong> personalizados.
      </p>
      ...
      <div style={S.card}>
        ...
        <label style={{ ...S.label, marginTop: 16 }}>
          Cole o texto da aula ou material:
        </label>
        <textarea
          style={S.textarea}
          value={text}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
          ...
          <button
            style={{ ...S.btn, opacity: !text.trim() || analyzing ? 0.5 : 1 }}
            onClick={analyzeWithAI}
            disabled={!text.trim() || analyzing || !HAS_AI}
          >
      ...
      {result && (
        <div style={S.card}>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={S.sectionLabel}>Módulos Identificados</div>
            <div style={S.tagsRow}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {result.exercises.map((ex, i) => (
              <div key={i} style={S.exerciseCard}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 500, color: '#0F172A', marginBottom: 4 }}>{ex.question}</div>
                  <div style={{ fontSize: 13, color: '#22C55E', fontWeight: 600, marginBottom: 2 }}>✓ {ex.answer}</div>
                  <div style={{ fontSize: 12, color: '#64748B' }}>💡 {ex.tip}</div>
                </div>
              </div>
            ))}
          </div>
...
const S: Record<string, React.CSSProperties> = {
  pageTitle: { fontSize: 22, fontWeight: 600, color: '#0F172A', marginBottom: 8, letterSpacing: '-0.3px' },
  pageDesc: { fontSize: 14, color: '#64748B', marginBottom: 24, lineHeight: 1.6 },
  ...
  card: { background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 14, padding: '24px', marginBottom: 20, boxShadow: '0 1px 4px rgba(15,23,42,0.05)' },
  label: { display: 'block', fontSize: 13, fontWeight: 600, color: '#334155', marginBottom: 6 },
  input: { width: '100%', padding: '10px 14px', border: '1px solid #E2E8F0', borderRadius: 8, fontSize: 14, fontFamily: 'inherit', color: '#0F172A', outline: 'none', boxSizing: 'border-box' },
  textarea: { width: '100%', padding: '12px 14px', border: '1px solid #E2E8F0', borderRadius: 8, fontSize: 14, fontFamily: 'inherit', color: '#0F172A', outline: 'none', resize: 'vertical', boxSizing: 'border-box', lineHeight: 1.6 },
  ...
  resultTitle: { fontSize: 16, fontWeight: 600, color: '#0F172A', margin: 0 },
  ...
  sectionLabel: { fontSize: 11, fontWeight: 700, color: '#94A3B8', letterSpacing: '0.6px', textTransform: 'uppercase', marginBottom: 8 },
  ...
  exerciseCard: { display: 'flex', gap: 12, padding: '14px', background: '#F8FAFC', borderRadius: 10, border: '1px solid #E2E8F0' },
  ...
  savedCard: { display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: '#FFFFFF', borderRadius: 10, border: '1px solid #E2E8F0' },
}
